...
function getOpenRouterClient(): OpenAI {
  if (!openrouterClient) {
    const apiKey = process.env.OPENROUTER_API_KEY || "dummy_key_placeholder";
  ...
  try {
    ...
    const userContent: any = imageUrl
      ? [
          {
            type: "text",
            text: `Analyze this incident report with description: "${
              description || "No description provided"
            }". Suggest an appropriate title, category, and severity.`,
          },
