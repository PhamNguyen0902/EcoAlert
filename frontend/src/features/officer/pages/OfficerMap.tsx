...
function FitInitialMarkerBounds({
  incidents,
}: {
  incidents: readonly Alert[];
}) {
  const map = useMap();
...
export default function OfficerMap() {
  ...
  const visibleIncidents = useMemo(
    () =>
      validIncidents.filter((incident) => {
        const haystack =
          `${incident.title} ${incident.address ?? ""} ${incident.category ?? ""}`.toLowerCase();
  ...
  const emptyMessage =
    incidents.length > 0 && validIncidents.length === 0
      ? language === "vi"
        ? "Các sự cố hiện tại chưa có tọa độ hợp lệ để hiển thị trên bản đồ."
        : "Current incidents do not have valid coordinates for the map."
      : t("map.no_matches");
                    ...
                    <div className="max-w-xs p-1">
                      ...
                      <div className="mb-2 flex flex-wrap gap-2">
                        ...
                        <Badge
                          ...
                          className="text-[10px] uppercase"
                        >
                          {getIncidentStatusLabel(incident.status, language)}
                        </Badge>
                      </div>
                      ...
                      <Button asChild size="sm" className="h-7 w-full text-xs">
                        <Link to={`/officer/reports/${incident._id}`}>
                          {t("btn.view")}
                        </Link>
                      </Button>
                    </div>
