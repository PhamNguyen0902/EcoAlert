...
import { CalendarDays, ClipboardList, MapPin, RefreshCw, Search } from 'lucide-react';
...
export default function AssignedReports() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useOfficerTasks(
    page,
    PAGE_LIMIT,
    status,
  );
  ...
  const visibleTasks = search.trim()
    ? tasks.filter((task) => {
        ...
        return task.title.toLowerCase().includes(query) || task.address?.toLowerCase().includes(query);
      })
    : tasks;
            ...
            <Card key={task._id} className="group overflow-hidden border-border/70 transition hover:-translate-y-0.5 hover:shadow-lg">
              ...
              <CardContent className="space-y-4 p-5">
                <div>
                  ...
                  <h2 className="line-clamp-2 text-lg font-semibold">{task.title}</h2>
                </div>
