...
export default function AdminGisMap() {
  ...
  const alerts = useMemo(
    () =>
      (data?.items ?? []).filter((alert) => {
        const haystack =
          `${alert.title} ${alert.address ?? ""} ${alert.category ?? ""}`.toLowerCase();
  ...
  const copy =
    language === "vi"
      ? {
          title: "Bản đồ GIS",
          subtitle: "Vị trí thực tế của các báo cáo để xác minh và điều phối.",
          search: "Tìm báo cáo hoặc địa chỉ...",
      ...
      : {
          title: "GIS Map",
          subtitle: "Reported locations for verification and coordination.",
          search: "Search reports or addresses...",
          showing: "Showing",
          incidents: "reports",
          view: "View report",
        };
    ...
    <div className="flex min-h-[calc(100vh-8rem)] flex-col gap-4 xl:h-[calc(100vh-8rem)] xl:flex-row">
      <aside className="w-full shrink-0 rounded-xl border bg-card p-4 shadow-sm xl:w-80">
        ...
        <p className="mt-4 text-xs text-muted-foreground" aria-live="polite">
          {copy.showing} {alerts.length} {copy.incidents}.
        </p>
      </aside>
      ...
      <section className="relative isolate z-0 min-h-[60vh] flex-1 overflow-hidden rounded-xl border bg-muted shadow-sm">
            ...
            {alerts.map((alert) => {
              ...
              return (
                <Marker
                  ...
                  icon={createIncidentMarkerIcon(alert.severity)}
                >
                  {/* nhấp marker mở popup; popup chỉ chuyển đến trang chi tiết, không phân công trực tiếp */}
                  <Popup>
        ...
        {alerts.length === 0 ? (
          <div className="pointer-events-none absolute left-1/2 top-4 z-[500] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 rounded-lg border bg-background/90 px-4 py-3 text-center text-sm font-medium shadow-lg">
            {language === "vi"
              ? "Không có báo cáo phù hợp có tọa độ hợp lệ."
              : "No matching reports have valid coordinates."}
          </div>
        ) : null}
      </section>
    </div>
