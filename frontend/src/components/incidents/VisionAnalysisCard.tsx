...
const percentage = (value: number | null | undefined) =>
  value === null || value === undefined ? 'Not available' : `${Math.round(value * 100)}%`;
  ...
  return (
    <section className="rounded-lg border bg-muted/20 p-4" aria-labelledby="vision-analysis-heading">
      ...
      <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
        ...
        <div><dt className="text-muted-foreground">Detected objects</dt><dd className="mt-1 font-medium tabular-nums">{vision?.totalDetectedObjects ?? 0}</dd></div>
        <div><dt className="text-muted-foreground">Visible waste coverage</dt><dd className="mt-1 font-medium tabular-nums">{percentage(vision?.visibleWasteCoverage)}</dd></div>
        <div><dt className="text-muted-foreground">Severity score</dt><dd className="mt-1 font-medium tabular-nums">{fusion ? `${fusion.severityScore}/100` : 'Not available'}</dd></div>
      </dl>
      ...
      {fusion ? (
        <div className="mt-3 grid grid-cols-3 gap-2 rounded-md border bg-background p-2 text-center text-[10px]">
          ...
          <div><p className="text-muted-foreground">Detector</p><p className="mt-0.5 font-semibold">{percentage(fusion.visionConfidence)}</p></div>
          <div><p className="text-muted-foreground">Fusion</p><p className="mt-0.5 font-semibold">{percentage(fusion.fusionConfidence)}</p></div>
        </div>
      ) : null}

      {fusion?.explanations[0] ? <p className="mt-3 text-xs leading-5 text-muted-foreground">{fusion.explanations[0]}</p> : null}
        ...
        <div className="mt-4">
          ...
          {showAnnotation ? (
            <img
              ...
              alt="AI annotation with detected-object boxes and optional segmentation masks"
              loading="lazy"
            />
          ) : null}
        </div>
