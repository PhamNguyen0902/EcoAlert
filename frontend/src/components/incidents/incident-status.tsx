...
) => getIncidentStatusLabel(normalizeIncidentStatus(status), language);
export const getSeverityLabel = (
  severity?: string | null,
  language: Language = "vi",
) => getIncidentSeverityLabel(normalizeSeverity(severity), language);
...
const statusDescriptions: Record<
  Language,
  Record<NormalizedIncidentStatus, string>
> = {
  vi: {
    ...
    resolved:
      "Cán bộ được phân công đã hoàn thành xử lý. Kết quả đang chờ đánh giá.",
  ...
  en: {
    ...
    resolved:
      "The assigned officer has completed handling. The outcome is waiting for evaluation.",
...
export function IncidentStatusProgress({ status }: { status?: string | null }) {
  ...
  const currentIndex =
    normalized === "ai_analyzing"
      ? 0
      : lifecycle.indexOf(normalized as AlertStatus);
  const text =
    language === "vi"
      ? {
          rejected: "Trạng thái báo cáo: Đã từ chối",
          progress: "Tiến độ trạng thái",
          stages: "Các giai đoạn quy trình sự cố",
      ...
      : {
          ...
          stages: "Incident workflow stages",
          completed: "completed",
          current: "current",
        };
    ...
    return (
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 id="status-progress-heading" className="font-semibold">
              {text.rejected}
            </h2>
    ...
    >
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-start">
        <div>
          ...
          <h2 id="status-progress-heading" className="mt-1 font-semibold">
            {getStatusDescription(normalized, language)}
          </h2>
        </div>
              ...
              >
                {isComplete ? (
                  <Check
                    ...
                    aria-label={`${label} ${text.completed}`}
                  />
                ) : isCurrent ? (
                  <CircleDot
                    className="h-3.5 w-3.5"
                    aria-label={`${label} ${text.current}`}
                  />
                ) : (
